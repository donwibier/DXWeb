...
namespace DX.Utils.CronJobs
{
	public static class CronBuilder
	{
		...
		public static bool IsStopRequested(object cronObject)
		{
			if ((cronObject is CronObject) && ((CronObject)cronObject).IsStopRequested)
				return true;
			return false;
		}
		...
		{
			string value = String.Format("{0}-{1}", firstHourToTrigger, lastHourToTrigger);
			if (interval != 1)
			{
				value += "/" + interval;
			}
		...
		public static CronExpression CreateMonthlyTriggerOnHour(int triggerDay, int triggerHour)
		{
            CronExpression cronExpression = new CronExpression
            {
				Minutes = "0",
				Hours = String.Format("{0}", triggerHour),
				Days = String.Format("{0}", triggerDay),
				Months = "*",
				DaysOfWeek = "*"
			};
			return cronExpression;

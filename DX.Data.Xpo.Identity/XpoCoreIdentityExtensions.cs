...
namespace DX.Data.Xpo.Identity
{
	...
	public static class XpoCoreIdentityExtensions
    {
		...
		public static IdentityBuilder AddXpoIdentityRoleMapper<TRole, TXPORole>(this IdentityBuilder builder, XPDataMapper<string, TRole, TXPORole> mapper)
			where TRole : XPIdentityRole<string, TXPORole, XpoDxRoleClaim>, IDxRole<string>, IRole<string>
			where TXPORole : XpoDxRole, IDxRole<string>, IRole<string>
		{
			builder.Services.TryAddSingleton<XPDataMapper<string, TRole, TXPORole>>(mapper);
			return builder;
		}
		...
		public static IdentityBuilder AddXpoIdentityRoleMapper<TKey, TRole, TXPORole, TXPORoleClaim>(this IdentityBuilder builder, XPDataMapper<TKey, TRole, TXPORole> mapper)
			where TKey : IEquatable<TKey>
			where TRole : XPIdentityRole<TKey, TXPORole, TXPORoleClaim>, IDxRole<TKey>, IRole<TKey>
			where TXPORole : XpoDxRole, IDxRole<TKey>, IRole<TKey>
			where TXPORoleClaim : XpoDxRoleClaim, IDxRoleClaim<TKey>
		{
		...
		public static IdentityBuilder AddXpoIdentityUserMapper<TUser, TXPOUser>(this IdentityBuilder builder, XPDataMapper<string, TUser, TXPOUser> mapper)
			where TUser : XPIdentityUser<string, TXPOUser>, IDxUser<string>, IUser<string>
			where TXPOUser : XpoDxUser, IDxUser<string>, IUser<string>
		{
			builder.Services.TryAddSingleton<XPDataMapper<string, TUser, TXPOUser>>(mapper);
			return builder;
		}

		public static IdentityBuilder AddXpoIdentityUserMapper<TKey, TUser, TXPOUser>(this IdentityBuilder builder, XPDataMapper<TKey, TRole, TXPOUser> mapper)
			where TKey : IEquatable<TKey>
			where TRole : XPIdentityUser<TKey, TXPOUser>, IDxUser<TKey>, IUser<TKey>
			where TXPOUser : XpoDxUser, IDxUser<TKey>, IUser<TKey>
		{
			builder.Services.TryAddSingleton<XPDataMapper<TKey, TRole, TXPOUser>>(mapper);
			return builder;
		}

        public static IdentityBuilder AddXpoIdentityStores<TUser, TXPOUser, TRole, TXPORole>(this IdentityBuilder builder)
			where TUser : IdentityUser, IDxUser<string>, IUser<string>
			where TRole : XpoDxRole, IDxRole<string>, IRole<string>
			where TXPOUser : XpoDxUser, IDxUser<string>, IUser<string>
            where TXPORole : XpoDxRole, IDxRole<string>, IRole<string>
        {
            return AddXpoIdentityStores<string, TXPOUser, XpoDxUserLogin, XpoDxUserClaim, XpoDxUserToken, TXPORole, XpoDxRoleClaim>(builder);
        }

        public static IdentityBuilder AddXpoIdentityStores<TKey, TUser, TRole, TXPOUser, TXPOUserLogin, TXPOUserClaim, TXPOUserToken, TXPORole, TXPORoleClaim>
            ...
            Type xpoRoleType, Type xpoRoleClaimType)
		{
			...
			services.TryAddScoped(typeof(IUserStore<>).MakeGenericType(userType/*, xpoUserType*/),
                (sp) =>
                {
                    ...
                    else
                    {
                        ...
                        return Activator.CreateInstance(userStoreType, connectionName);
                    }
                });
            services.TryAddScoped(typeof(IRoleStore<>).MakeGenericType(roleType/*, xpoRoleType*/),
                (sp) =>
                {
                    ...
                    {
                        return Activator.CreateInstance(roleStoreType, connectionName);
                    }
                });
        }
